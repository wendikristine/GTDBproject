...
WeaponUseByYear <- GTdt_NoDoubt_2000[ , sum(NumberofAttacks), by = list(iyear,weaptype1)]

WeaponType <- factor(WeaponUseByYear$weaptype1)
ggplot(WeaponUseByYear, aes(x= WeaponUseByYear$iyear, y= WeaponUseByYear$V1, color = WeaponType)) + geom_line() + xlab('Year') +ylab('Occurrence')

freq_df <- count(GTdt_NoDoubt_2000, 'country_txt')

most_freq <- subset(freq_df, freq > 3000)
kable(most_freq[order(-most_freq$freq),], row.names = FALSE, col.names = c("Country", "# of Terror Attacks"))
...
In results section you can start with an overview of what you have found during the exploration of data.
